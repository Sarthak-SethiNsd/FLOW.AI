...
// Easing function for smooth keyframe interpolation
...
export default function WatchClient({ asana }) {
  ...
  const [animCoords, setAnimCoords] = useState(
    data && data.steps ? data.steps[0] : null
  );
  ...
  const fromCoordsRef = useRef(data && data.steps ? data.steps[0] : null);
  const toCoordsRef = useRef(data && data.steps ? data.steps[0] : null);
  const animStartRef = useRef(null);

  // Visual alt-view timer refs
  ...
  return (
    <div className="min-h-screen md:h-screen flex flex-col md:overflow-hidden bg-background text-foreground select-none">
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
        ...
        {/* Left: Canvas */}
        <div className="h-[55vh] md:flex-1 md:h-auto p-4 md:p-5 flex flex-col relative bg-background">
          <div className="relative w-full h-full rounded-xl overflow-hidden bg-panel border border-border-dark flex items-center justify-center shadow-2xl">
            ...
            <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-20 w-2/3 text-center">
              ...
              <div className="w-full bg-gray-800 h-1.5 rounded-full overflow-hidden border border-gray-700">
                <div
                  className="bg-gradient-to-r from-flow-green to-emerald-400 h-full rounded-full shadow-[0_0_10px_#2ea44f] transition-all duration-500"
